    ...
    $.fn.datalist = function(options) {
        ...
        var client = {
            ...
            target: function(response) {
                if (response['invoice']) {
                    ...
                    if (response['more']) {
                        ...
                        client.updateTable(parseInt(response['offset']));
                    }
                }  else if (response['userLists']) {
                        $('thead').removeClass('hidden');

                        $.each(response['userLists'], function(key, value) {
                ...
                } else if (response['datanalysis']) {
                    $('thead').removeClass('hidden');

                    if(response['head']){
                        // $('#portfolio-flters').html('<li data-filter=".filter-'+response['head2']+'">'+response['head2']+'</li>');
                    }
                    $.each(response['datanalysis'], function(key, value) {
            ...
            // Finance starts
                ...
                else if (response['userinvoice']) {
                    $('thead').removeClass('hidden');

                    $.each(response['userinvoice'], function(key, value) {
