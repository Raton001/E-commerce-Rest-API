...
if (isset($input['internal'])) {
  ...
  return view('orders', ['account'=>$account,'data'=>$_SESSION['orders'][$account][$userID][$date_start.'-'.$date_end],'stores'=>$this->stores(), 'source'=>'shopee', 'last_fetched'=>$_SESSION['orders'][$account][$userID]['last_fetched']]);
...
if (isset(array_column($sessionData, 'READY_TO_SHIP')[0])) {
  ...
  $orderIDs = array_column(array_column($sessionData, 'READY_TO_SHIP')[0], 'ordersn');
    ...
    if (isset($input['internal'])) {
     ...
     return view('orders', ['account'=>$account, 'accounts'=>$accounts, 'data'=>$sessionData[$date_start.'-'.$date_end],'stores'=>$this->stores(), 'source'=>'shopee', 'last_fetched'=>$_SESSION['orders'][$account][$userID]['last_fetched']]);
